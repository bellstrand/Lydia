...
class CMUser extends CObject implements IHasSQL, ArrayAccess{
   ...
   */
	...
	*/
	public function Init(){
		try{
			...
			$this->session->AddMessage('notice', 'Successfully created the database tables and created a default admin user as root:root and an ordinary user ad doe:doe.');
		}
		catch(Exception$e){
	...
	* @return boolean true of match else false.
	*/
	...
	* @param $algorithm string starting what algorithm to use, plain, md5, md5salt, sha1, sha1salt.
	...
	*/
	public function CreatePassword($plain, $algorithm=null){
		$password = array(
			'algorithm' => ($algorithm ? $algorithm : CLydia::Instance()->config['hashing_algorithm']),
			'salt' => null
		);
